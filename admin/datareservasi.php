...
$query = mysqli_query($connect, "SELECT p.*,u.*,a.*,d.nama,d.spesialis,d.no_hp AS no_hp_dokter,j.* FROM pasien p INNER JOIN users u ON p.id_user = u.id_user INNER JOIN antrian a ON a.id_pasien = p.id INNER JOIN dokter d ON p.id_dokter = d.id INNER JOIN jadwal_dokter j ON j.id_dokter = d.id");
